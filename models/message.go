...
func QueryMessageCountByNoteKey(notekey string)(count int64, err error){
	...
	count, err = db.QueryTable("message").Filter("NoteKey", "").OrderBy("-updated").Count()
	fmt.Printf("\n--models/message.go QueryMessageCountByNoteKey count: %v --\n\n", count)
...
func QueryPageMessageByNoteKey(notekey string, pageno int, pagesize int) (data []orm.Params, err error) {
	firstLimit := (pageno - 1) * pagesize
	//limit := page*limit
	fmt.Printf("---models/message.go--- QueryPageMessageByNoteKey  firstLimit:%v , pagesize: %v ", firstLimit, pagesize)

	db := orm.NewOrm()

	var maps []orm.Params
	_, err = db.QueryTable("message").Filter("NoteKey", notekey).OrderBy("-updated").Limit(pagesize, firstLimit).Values(&maps)
	if err := mapstructure.Decode(maps, &data); err != nil {
		fmt.Println(err)
	}
