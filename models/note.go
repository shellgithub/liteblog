...
func QueryNoteByKeyAndUserid(key string, userid int) (note Note, err error) {
	...
	err1 := qs.Filter("key", key).Filter("userID",userid).One(&note)
	if err1 != nil {
		fmt.Printf("\n--models/note.go 没查到 key: %v  --\n\n, err1: %v", key, err1)
	}else {
		fmt.Printf("\n--models/note.go 查到有 key: %v  --\n\n, err1: %v", key, err1)
...
func SaveNote(note *Note) error{
	fmt.Printf("\n--models/note.go 开始执行 SaveNote \n\n " +
		...
		note.Key, note.UserID, note.User, note.Title, note.Content)
	...
	if err == nil {
		if created {
			...
			fmt.Printf("Get an object. Title: %v, UserID: %v, Summary: %v, Content: %v, 开始更新note 内容\n\n", note.Title,note.UserID,note.Summary,note.Content)
